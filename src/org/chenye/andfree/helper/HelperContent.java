...
public class HelperContent extends AFLogObj{
	...
	private static Hashtable<String, HelperContent> instance = new Hashtable<String, HelperContent>();
	private Line _queryData = Line.def();
	public static HelperContent SMS(){
		return getInstance(TYPE_SMS, "sms");
	}
	
	public static HelperContent MMS(){
		return getInstance(TYPE_MMS, "pdu");
	}
	
	public static HelperContent MMS_PART(){
		return getInstance(TYPE_MMS_PART, "parts");
	}
	
	public static HelperContent THREAD(){
		return getInstance(TYPE_THREAD, "threads");
	}
	
	public static HelperContent CONTACT_PHONE(){
		return getInstance(TYPE_CONTACT_PHONE, "phones");
	}
	
	public static HelperContent CONTACT(){
		return getInstance(TYPE_CONTACT, "contact");
	}
	
	private static HelperContent getInstance(String type, final String table_name){
		if (instance.containsKey(type)) return instance.get(type);
		instance.put(type, new HelperContent(type){{
	...
	String virtualTable;
	String currentTable = "";
	public HelperContent(String mType){
	...
	public HelperContent distinct(String... fields){
		fields[0] = "DISTINCT " + fields[0];
		return select(fields);
	}
	
	public HelperContent select(String... fields){
		String str = "";
	...
	public HelperContent where(String where, Object... args){
		_queryData.put("where", String.format(where, args));
		return this;
	}

	public HelperContent order(String field){
		return order(field, "DESC");
	}
	
	public HelperContent order(String field, String sort){
		_queryData.put("order", field + " " + sort);
		return this;
	}
	
	public HelperContent limit(int count){
		return limit("0, " + count);
	}
	
	public HelperContent limit(String limit_string){
		_queryData.put("limit", limit_string);
		return this;
	}
	
	public HelperContent join(String table, String condition){
