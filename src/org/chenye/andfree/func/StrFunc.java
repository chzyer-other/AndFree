...
public class StrFunc extends BaseLog{
	...
	public static String get_all_phone(String phone){
		phone = StrFunc.Sub2Phone(phone);
		if ( ! check_phone(phone)) return "'" + phone + "'";
		if (StrFunc.check_phone(phone)){
	...
	public static String check_password(String body){
		if (body == null){
			error(new StrFunc(), "check_password-> null body");
			return "";
		}
		body = body.replaceAll("忘密码|忘记密码", "");
		String keyword = "(?:密码|验证码|邀请码|校验码|申请码|使用码|口令)";
		String result = "";
		Matcher m = StrFunc.preg_match(keyword + ".*?(\\d+)", body);
		if ( ! m.find()){
			return "";
		}
		...
		m = StrFunc.preg_match("(\\d+).*?" + keyword, body);
		if ( ! m.find() || m.group(1).length() < 4){
			return "";
		}
		result = m.group(1);
		m = StrFunc.preg_match(result + "\\.(cn|com)", body);
		if (m.find()) return "";
		return result;
	}
